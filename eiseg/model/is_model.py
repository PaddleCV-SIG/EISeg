...
class ISModel(nn.Layer):
    def __init__(self, use_rgb_conv=True, with_aux_output=False, with_edge=False,
                 with_finenet=False, norm_radius=5, use_disks=False, cpu_dist_maps=False,
        ...
        self.with_finenet = with_finenet
        ...
        if self.with_prev_mask or self.with_edge:
            self.coord_feature_ch += 1

        if self.with_finenet:
            self.finenet = FineNet(8, 1)
    ...
    def forward(self, image, coord_features_org):
        # image, prev_mask = self.prepare_input(image)
        # coord_features = self.get_coord_features(image, prev_mask, points)

        if self.rgb_conv is not None:
            x = self.rgb_conv(paddle.concat((image, coord_features_org), axis=1))
            outputs = self.backbone_forward(x)
        else:
            coord_features = self.maps_transform(coord_features_org)
            outputs = self.backbone_forward(image, coord_features)

        outputs['instances'] = nn.functional.interpolate(outputs['instances'], size=paddle.shape(image)[2:],
                                                         mode='bilinear', align_corners=True)
        print("&*&&&&&&&&&&&&&&")
        print(paddle.shape(outputs['instances']))
        print(paddle.shape(outputs['instances_edge']))
        print(paddle.shape(outputs['instances_aux']))
        print("&&&&&&&&&&&&&&&&&&&&&")

        if self.with_aux_output:
            outputs['instances_aux'] = nn.functional.interpolate(outputs['instances_aux'], size=paddle.shape(image)[2:],
                                                                 mode='bilinear', align_corners=True)

        if self.with_edge:
            outputs['instances_edge'] = nn.functional.interpolate(outputs['instances_edge'], size=paddle.shape(image)[2:],
                                                                 mode='bilinear', align_corners=True)
            print("**************")
            print(paddle.shape(image))
            print(paddle.shape(coord_features[:,1:, :,:]))
            print(paddle.shape(outputs['instances_edge']))
            print(paddle.shape(outputs['instances']))

        if self.with_finenet:
            fineout = self.finenet(image, coord_features_org, outputs['instances'], outputs['instances_edge'])
            outputs['instances'] = fineout
