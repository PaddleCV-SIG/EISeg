...
def parse_args():
    ...
    parser.add_argument(
        "--config", dest="cfg", type=str, \
        default='iann/train/train_config.yaml', help='The config file.')
...
def main():
    ...
    logger.info(info)
    place = 'gpu' if env_info['Paddle compiled with cuda'] and \
            env_info['GPUs used'] else 'cpu'
    ...
    model_cfg.input_transform = T.Compose([
        T.Normalize(mean=model_cfg.input_normalization['mean'],
    ...
    if models.get('type') == 'deeplab':
        model = get_deeplab_model(backbone=models.get('backbone'), is_ritm=models.get('is_ritm'))
    elif models.get('type') == 'hrnet':
        model = get_hrnet_model(width=models.get('width'), ocr_width=models.get('ocr_width'), \
                                with_aux_output=models.get('with_aux_output'), is_ritm=models.get('is_ritm'))
    ...
    if nranks > 1:
        if not paddle.distributed.parallel.parallel_helper._is_parallel_ctx_initialized():
            ...
            ddp_net = paddle.DataParallel(model)
        train(ddp_net, cfg, model_cfg, backbone_params=backbone_params,
              other_params=other_params)
    else:
        train(model, cfg, model_cfg, backbone_params=backbone_params,
              other_params=other_params)
