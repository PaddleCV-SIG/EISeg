...
class NormalizedFocalLossSigmoid(nn.Layer):
    ...
    def forward(self, pred, label, sample_weight=None):
        ...
        ignore_area = paddle.sum(
            ...
            axis=tuple(range(1, len(label.shape))),
        ).numpy()
        ...
        if self._size_average:
            bsum = paddle.sum(
                sample_weight,
                axis=U.get_dims_with_exclusion(
                    len(sample_weight.shape), self._batch_axis
                ),
            )
            loss = paddle.sum(
                loss, axis=U.get_dims_with_exclusion(len(loss.shape), self._batch_axis)
            ) / (bsum + self._eps)
        else:
            loss = paddle.sum(
                loss, axis=U.get_dims_with_exclusion(len(loss.shape), self._batch_axis)
...
class FocalLoss(nn.Layer):
    ...
    def forward(self, pred, label, sample_weight=None):
        ...
        loss = (
            -alpha
            * beta
            * paddle.log(paddle.min(pt + self._eps, paddle.ones(1, dtype="float32")))
        )
        ...
        if self._size_average:
            tsum = paddle.sum(
                label == 1,
                axis=U.get_dims_with_exclusion(len(label.shape), self._batch_axis),
            )
            loss = paddle.sum(
                loss, axis=U.get_dims_with_exclusion(len(loss.shape), self._batch_axis)
            ) / (tsum + self._eps)
...
class SigmoidBinaryCrossEntropyLoss(nn.Layer):
    ...
    def forward(self, pred, label):
        ...
        if not self._from_sigmoid:
            ...
            eps = 1e-12
            loss = -(
                paddle.log(pred + eps) * label
                + paddle.log(1.0 - pred + eps) * (1.0 - label)
            )
        loss = self._weight * (loss * sample_weight)
        return paddle.mean(
            loss, axis=U.get_dims_with_exclusion(len(loss.shape), self._batch_axis)
