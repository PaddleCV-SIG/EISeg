...
from model import DistMapsHRNetModel
...
class BRSBasePredictor(BasePredictor):
    ...
    def _get_clicks_maps_nd(self, clicks_lists, image_shape, radius=1):
        pos_clicks_map = np.zeros((len(clicks_lists), 1) + tuple(image_shape), dtype=np.float32)
        neg_clicks_map = np.zeros((len(clicks_lists), 1) + tuple(image_shape), dtype=np.float32)
...
class FeatureBRSPredictor(BRSBasePredictor):
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed, mask):
        ...
        self.opt_functor.init_click(get_prediction_logits, self.model, pos_mask, neg_mask)
    ...
    def _get_head_input(self, image_nd, points, mask=None):
        with paddle.no_grad():
            ...
            x = self.net.rgb_conv(paddle.concat((image_nd, mask, coord_features), axis=1))
...
class InputBRSPredictor(BRSBasePredictor):
    ...
    def _get_prediction(self, image_nd, clicks_lists, is_image_changed):
        ...
        if num_clicks > self.optimize_after_n_clicks:
            opt_result = fmin_l_bfgs_b(func=self.opt_functor, x0=self.opt_data.numpy().ravel(),
                                       **self.opt_functor.optimizer_params)
