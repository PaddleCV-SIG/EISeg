...
class ZoomIn(BaseTransform):
    ...
    def transform(self, image_nd, clicks_lists, mask=None):
        ...
        if update_object_roi:
            ...
            self._roi_mask = get_roi_image_nd(
                paddle.to_tensor(self._prev_probs), self._object_roi, self.target_size
            )
            self.image_changed = True

        tclicks_lists = [self._transform_clicks(clicks_list)]
        return self._roi_image, tclicks_lists, self._roi_mask
