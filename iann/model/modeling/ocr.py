...
class SpatialGather_Module(nn.Layer):
    ...
    def forward(self, feats, probs):
        ...
        m = paddle.matmul(probs, feats)
        ocr_context = paddle.unsqueeze(
            paddle.matmul(probs, feats).transpose([0, 2, 1]), 3
        )
        return ocr_context
...
class SpatialOCR_Module(nn.Layer):
    ...
    def __init__(
        ...
        self.conv_bn_dropout = nn.Sequential(
            nn.Conv2D(
                _in_channels, out_channels, kernel_size=1, padding=0, bias_attr=False
            ),
            nn.Sequential(is_bn(norm_layer, out_channels), nn.ReLU()),
...
class ObjectAttentionBlock2D(nn.Layer):
    ...
    def __init__(
        ...
        self.f_pixel = nn.Sequential(
            ...
            nn.Sequential(is_bn(norm_layer, self.key_channels), nn.ReLU()),
            nn.Conv2D(
        ...
        self.f_up = nn.Sequential(
            ...
            nn.Sequential(is_bn(norm_layer, self.in_channels), nn.ReLU()),
        )
    ...
    def forward(self, x, proxy):
        ...
        if self.scale > 1:
            context = F.interpolate(
                context, size=[h, w], mode="bilinear", align_corners=self.align_corners
            )
        return context
